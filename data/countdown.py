"""
Countdown dataset generation, solving, and search trajectories.
Consolidated single-file implementation containing utilities, search algorithms, and generation script.
"""

import os
import re
import json
import random
import heapq
import itertools
import argparse
import tiktoken
import tqdm


# ==============================================================================
# Helper Functions & Node Classes
# ==============================================================================

def combine_nums(a, b, max_intermediate=None):
    """
    Combines two numbers using addition, multiplication, subtraction, and division.
    Keeps track of equations as strings.
    """
    a = int(a)
    b = int(b)
    possible = []
    
    if max_intermediate is not None:
        if a + b < max_intermediate:
            possible.append([a + b, f"{a}+{b}={a+b}"])
        if a * b < max_intermediate:
            possible.append([a * b, f"{a}*{b}={a*b}"])
    else:
        possible = [[a + b, f"{a}+{b}={a+b}"], [a * b, f"{a}*{b}={a*b}"]]

    if a <= b:
        possible.append([b - a, f"{b}-{a}={b-a}"])
        if a != 0 and b % a == 0:
            possible.append([b // a, f"{b}/{a}={b//a}"])
    else:
        possible.append([a - b, f"{a}-{b}={a-b}"])
        if b != 0 and a % b == 0:
            possible.append([a // b, f"{a}/{b}={a//b}"])
            
    return possible


class CountdownNode:
    """
    Represents a node in the BFS/DFS search tree.
    """
    def __init__(self, idx, parent, nums, operations, heuristic_val):
        self.nums = nums
        self.operations = operations
        self.heuristic = heuristic_val
        self.parent = parent
        self.idx = idx

    def __lt__(self, other):
        return self.heuristic < other.heuristic


# ==============================================================================
# Heuristics & Pruning
# ==============================================================================

def sum_heuristic(nums, target):
    """
    Heuristic based on average absolute distance from numbers to target.
    """
    if len(nums) == 1:
        return abs(nums[0] - target)
    return sum(abs(num - target) for num in nums) / len(nums)


def mult_heuristic(nums, target):
    """
    Heuristic based on average absolute distance from numbers to factors of target.
    """
    factors = [i for i in range(2, target + 1) if target % i == 0]
    if not factors:
        factors = [1, target]
    return sum(min(abs(num - factor) for factor in factors) for num in nums)


def great_prune(heuristic_val, target):
    """
    Prune if the heuristic exceeds the target value.
    """
    return heuristic_val > target


def mult_prune(result, target):
    """
    Prune if result is not close to a factor of target.
    """
    factors = [i for i in range(1, target + 1) if target % i == 0]
    return all(abs(result - factor) > target for factor in factors)


# ==============================================================================
# Evaluation Metrics & Parsers
# ==============================================================================

def simple_rating(search_path):
    """
    Calculates the number of nodes explored in a search path trace.
    """
    nodes_explored = search_path.count("Exploring Operation") + 1
    return nodes_explored


def get_target_nums(search_path, mode=""):
    """
    Extracts the target and initial numbers from the first line of search path trace.
    """
    first_line = search_path.strip().split("\n")[0]
    search_path = search_path.replace("<|endoftext|>", "")
    if mode == "dt":
        first_line = first_line.split("->")[1]
    target_nums_match = re.match(r"Current State: (\d+):\[(.*?)\]", first_line)
    if not target_nums_match:
        return "Invalid input: Cannot find the initial state in the first line."

    target, nums = int(target_nums_match.group(1)), [
        int(n) for n in target_nums_match.group(2).split(", ")
    ]
    return target, nums


def parse_trajectory(search_path, mode="dt"):
    """
    Parses search trajectory and verifies if it is a valid solution path.
    """
    first_line = search_path.strip().split("\n")[0]
    search_path = search_path.replace("<|endoftext|>", "")

    target_nums_match = re.match(r"Current State: (\d+):\[(.*?)\]", first_line)
    if not target_nums_match:
        return "Invalid input: Cannot find the initial state in the first line."

    target, nums = int(target_nums_match.group(1)), [
        int(n) for n in target_nums_match.group(2).split(", ")
    ]

    goal_lines = list(re.finditer(r"\d+,\d+ equal: Goal Reached", search_path))
    if not goal_lines:
        return "No goal reached statement found."

    goal_line = goal_lines[0]
    operations = re.findall(
        r"Exploring Operation: (.*?=\d+), Resulting Numbers: \[(.*?)\]",
        search_path[: goal_line.start()],
    )
    if not operations:
        return "No operations found leading to the goal."

    final_operation = operations[-1][0]
    try:
        predicted_result = int(final_operation.split("=")[1])
    except Exception:
        return "Could not parse last operation"
        
    if predicted_result != target:
        return "Invalid path: Final operation does not result in target."

    state_lines = re.findall(
        r"Current State: \d+:\[.*?\], Operations: \[(.*?)\]",
        search_path[: goal_line.start()],
    )
    if not state_lines:
        return "No intermediate states found."
        
    operation_list = state_lines[-1].split(", ")
    operation_list = [op.replace("'", "").strip() for op in operation_list if op]
    operation_list += [final_operation]

    available_numbers = nums[:]
    for operation in operation_list:
        try:
            left, right = operation.split("=")
        except ValueError:
            return "Could not split operation into lhs, rhs"
        try:
            # Simple eval verification (safe since we only have basic math symbols)
            if eval(left) != int(right):
                return f"Invalid operation: {operation}"
        except Exception as e:
            return f"Error in evaluating operation {operation}: {e}"
            
        used_numbers = re.findall(r"\d+", left)
        for n in used_numbers:
            val = int(n)
            if val not in available_numbers:
                return f"Invalid operation: {operation}, number {val} not available in {available_numbers}"
            available_numbers.remove(val)
        available_numbers.append(int(right))

    return "Valid path."


def metric_fn(search_path, mode="dt"):
    """
    Computes performance rating of search trajectory.
    """
    rating = parse_trajectory(search_path, mode=mode)
    if rating == "Valid path.":
        score = simple_rating(search_path)
        first_line = search_path.strip().split("\n")[0]
        if "->" in first_line:
            first_line = first_line.split("->")[1]
        target_nums_match = re.match(r"Current State: (\d+):\[(.*?)\]", first_line)
        target, nums = int(target_nums_match.group(1)), [
            int(n) for n in target_nums_match.group(2).split(", ")
        ]
        
        n_nums = len(nums)
        if n_nums == 2:
            max_nodes = 4
        elif n_nums == 3:
            max_nodes = 48
        elif n_nums == 4:
            max_nodes = 1152
        elif n_nums == 5:
            max_nodes = 46080
        else:
            max_nodes = 100000
            
        return (max(1.0 - score / max_nodes, 0.0), rating)
    return (0.0, rating)


# ==============================================================================
# Search Tree classes (for structural analysis)
# ==============================================================================

class SearchTreeNode:
    def __init__(self, options, op, id_val):
        self.options = options
        self.op = op
        self.id = id_val


class SearchTree:
    def __init__(self):
        self.data = dict()
        self.target = -1
        self.correctness = 0
        self.goal_reached = 0
        self.final_op_correct = 0
        self.num_total_nodes = 0
        self.correct_path = []
        self.num_nodes_in_correct_path = 0
        self.rating = 0
        self.max_nodes = 0
        self.errors = {
            "arithmetic": {},
            "exploration": {},
            "formatting": {},
            "other": {},
        }
        self.path = []

    def print_tree(self, data, indent):
        if data == self.data:
            print(self.data)
            print(
                "Node 0:  Target: ",
                self.target,
                "Nums: ",
                self.data[0][0].options,
                "Correctness: ",
                self.correctness,
                "Rating: ",
                self.rating,
            )
            self.print_tree(data[0][1], 4)
        else:
            for key, node_and_dict in data.items():
                print(" " * indent + str(key), end=": ")
                if bool(node_and_dict[1]):
                    print(node_and_dict[0].options, node_and_dict[0].op)
                    self.print_tree(node_and_dict[1], indent + 4)
                else:
                    print(node_and_dict[0].options, node_and_dict[0].op)

    def parse_search_trajectory(self, search_path, mode="sft"):
        first_line = search_path.strip().split("\n")[0]
        search_path = search_path.replace("<|endoftext|>", "")
        if "Goal Reached" in search_path:
            index = search_path.find("Goal Reached")
            search_path = search_path[: index + len("Goal Reached") + 1]

        target_nums_match = re.search(
            r"Current State: (\d+):\[(.*?)\]", first_line.replace("<|endoftext|>", "")
        )
        if not target_nums_match:
            return "Invalid input: Cannot find the initial state in the first line."

        target = int(target_nums_match.group(1))
        nums = json.loads("[" + target_nums_match.group(2) + "]")
        self.target = target
        self.data[0] = [SearchTreeNode(nums, None, tuple([0])), dict()]

        if len(nums) == 3:
            self.max_nodes = 48
        elif len(nums) == 4:
            self.max_nodes = 1152

        goal_lines = list(re.finditer(r"\d+,\d+ equal: Goal Reached", search_path))
        if goal_lines:
            self.goal_reached = 1

        if self.goal_reached == 1:
            goal_line = goal_lines[0]
            operations = re.findall(
                r"Exploring Operation: (.*?=\d+), Resulting Numbers: \[(.*?)\]",
                search_path[: goal_line.start()],
            )
            if not operations:
                self.errors["exploration"][tuple([0])] = "No operations found leading to the goal."
                return
        else:
            operations = re.findall(
                r"Exploring Operation: (.*?=\d+), Resulting Numbers: \[(.*?)\]",
                search_path,
            )
            if not operations:
                self.errors["exploration"][tuple([0])] = "No operations found."
                return

        if self.goal_reached == 1:
            search_path = search_path[: goal_line.end()]

        node_list = re.findall(
            r"(Generated) Node #\d+((,\d+)*)(\n)*( )*(Current State)*: (\d*):\[(.*)?\](,)* Operation(s)*: (\[)*(.*)(\])*",
            search_path,
        )

        self.num_total_nodes += 1

        for node in node_list:
            node_id = [int(num) for num in node[1].split(",")[1:]]
            if node[0] == "Generated" and node_id != []:
                self.num_total_nodes += 1
                operation_str = node[-2]
                operation = re.search(r"\d+[-+*\/]\d+=\d+", operation_str)
                if not operation:
                    self.errors["formatting"][tuple([0] + node_id)] = f"Invalid operation {operation_str}."
                    break
                op = operation.group(0)
                try:
                    left, right = op.split("=")
                except Exception:
                    self.errors["formatting"][tuple([0] + node_id)] = f"Could not parse or split operation {op} into lhs, rhs."
                    continue
                try:
                    if eval(left) != int(right):
                        self.errors["arithmetic"][tuple([0] + node_id)] = f"Invalid operation {op}."
                except Exception as e:
                    if tuple([0] + node_id) not in self.errors["other"]:
                        self.errors["other"][tuple([0] + node_id)] = [f"Error in evaluating operation {op}: {e}."]
                    else:
                        self.errors["other"][tuple([0] + node_id)].append([f"Error in evaluating operation {op}: {e}."])
                
                exploration_error_on_this_node = 0
                subdict = self.data[0]
                for key in node_id[:-1]:
                    if key not in subdict[1]:
                        self.errors["exploration"][tuple([0] + node_id)] = f"Exploration is jumping steps. {key} does not exist in {subdict[1]}."
                        exploration_error_on_this_node = 1
                        break
                    subdict = subdict[1][key]

                if exploration_error_on_this_node == 0:
                    used_numbers = re.findall(r"\d+", left)
                    parent_options = subdict[0].options.copy()
                    for used_num in used_numbers:
                        if int(used_num) not in parent_options:
                            if tuple([0] + node_id) not in self.errors["other"]:
                                self.errors["other"][tuple([0] + node_id)] = [f"Invalid operation: {op}, number {used_num} not available in {parent_options}."]
                            else:
                                self.errors["other"][tuple([0] + node_id)].append(f"Invalid operation: {op}, number {used_num} not available in {parent_options}")

                    for used_num in used_numbers:
                        if int(used_num) in parent_options:
                            parent_options.remove(int(used_num))
                    try:
                        remaining_options = list(int(num) for num in node[7].split(","))
                    except Exception:
                        self.errors["formatting"][tuple([0] + node_id)] = f"Could not parse or split remaining options {node[7]}."
                        continue
                    if not set([number for number in parent_options if number not in used_numbers] + [int(right)]):
                        if tuple([0] + node_id) not in self.errors["other"]:
                            self.errors["other"][tuple([0] + node_id)] = [f"Error in remaining options: {subdict[0].options} should be {parent_options + [int(right)]} instead."]
                        else:
                            self.errors["other"][tuple([0] + node_id)].append(f"Error in remaining options: {subdict[0].options} should be {parent_options + [int(right)]} instead.")

                    new_node = SearchTreeNode(
                        list(int(num) for num in node[7].split(",")),
                        op,
                        tuple([0] + node_id),
                    )
                    if node_id[-1] not in subdict[1]:
                        subdict[1][node_id[-1]] = [new_node, dict()]
                    else:
                        self.errors["exploration"][tuple([0] + node_id)] = [f"Trying to generate node with index that has already been generated: {tuple([0] + node_id)} already exists."]

        failed_final_explorations = re.findall(
            r"Moving to Node \#((\d+,)*\d+)\n.*\nExploring Operation: (\d+)([-+\/*])(\d+)=(\d+).*\n.*unequal: No Solution",
            search_path,
        )
        for exploration in failed_final_explorations:
            node_id = [int(num) for num in exploration[0].split(",")[1:]]
            op_num_1, operator, op_num_2, op_num_result = (
                int(exploration[-4]),
                exploration[-3],
                int(exploration[-2]),
                int(exploration[-1]),
            )
            try:
                if (operator == "/" and op_num_2 == 0) or eval(f"{op_num_1}{operator}{op_num_2}") != int(op_num_result):
                    self.errors["arithmetic"][tuple([0] + node_id)] = f"Invalid operation {op_num_1}{operator}{op_num_2}={op_num_result}."
            except Exception as e:
                if tuple([0] + node_id) not in self.errors["other"]:
                    self.errors["other"][tuple([0] + node_id)] = [f"Error in evaluating operation {op_num_1}{operator}{op_num_2}={op_num_result}: {e}."]
                else:
                    self.errors["other"][tuple([0] + node_id)].append([f"Error in evaluating operation {op_num_1}{operator}{op_num_2}={op_num_result}: {e}."])

            exploration_error = 0
            self.num_total_nodes += 1
            subdict = self.data[0]
            for key in node_id[:-1]:
                if key not in subdict[1]:
                    self.errors["exploration"][tuple([0] + node_id)] = f"Exploration is jumping steps. {key} does not exist in {subdict[1]}."
                    break
                subdict = subdict[1][key]
            if node_id:
                if node_id[-1] in subdict[1]:
                    subdict = subdict[1][node_id[-1]]
                else:
                    self.errors["exploration"][tuple([0] + node_id)] = f"Exploration is jumping steps. {node_id[-1]} does not exist in {subdict[1]}."
                    exploration_error = 1
            if exploration_error == 0:
                op = f"{op_num_1}{operator}{op_num_2}={op_num_result}"
                op_node = SearchTreeNode([op_num_result], op, tuple([0] + node_id))
                if subdict[1]:
                    max_leaf_index = max(subdict[1].keys())
                    subdict[1][max_leaf_index + 1] = [op_node, dict()]
                    node_id = node_id + [max_leaf_index + 1]
                else:
                    subdict[1][0] = [op_node, dict()]
                    node_id = node_id + [0]
                if (operator == "/" and op_num_2 == 0) or eval(f"{op_num_1}{operator}{op_num_2}") != op_num_result:
                    self.errors["arithmetic"][tuple([0] + node_id)] = f"Invalid operation: {op}."

                used_numbers = [op_num_1, op_num_2]
                parent_options = subdict[0].options.copy()
                for used_num in used_numbers:
                    if int(used_num) not in parent_options:
                        if tuple([0] + node_id) not in self.errors["other"]:
                            self.errors["other"][tuple([0] + node_id)] = [f"Invalid operation: {op}, number {used_num} not available in {parent_options}."]
                        else:
                            self.errors["other"][tuple([0] + node_id)].append(f"Invalid operation: {op}, number {used_num} not available in {parent_options}")

                for used_num in used_numbers:
                    if int(used_num) in parent_options:
                        parent_options.remove(int(used_num))
                if parent_options:
                    if tuple([0] + node_id) not in self.errors["other"]:
                        self.errors["other"][tuple([0] + node_id)] = [f"Error in remaining options: remaining nums should be [] instead."]
                    else:
                        self.errors["other"][tuple([0] + node_id)].append(f"Error in remaining options: remaining nums should be [] instead.")

        exploration_list = re.findall(
            r"(Moving to) Node #\d+((,\d+)*)(\n)*( )*(Current State)*: (\d*):\[(.*)?\]\n(,)*.*Operation(s)*: (\d+)[-+\/*](\d+)=(\d+)",
            search_path,
        )
        if exploration_list and self.goal_reached:
            last_exploration = exploration_list[-1]
            node_id = [int(num) for num in last_exploration[1].split(",")[1:]]
            self.num_total_nodes += 1
            subdict = self.data[0]
            exploration_error_on_final_node = 0
            for key in node_id[:-1]:
                if key not in subdict[1]:
                    self.errors["exploration"][tuple([0] + node_id)] = f"Exploration is jumping steps. {key} does not exist in {subdict[1]}."
                    exploration_error_on_final_node = 1
                    break
                subdict = subdict[1][key]
            if node_id:
                if node_id[-1] in subdict[1]:
                    subdict = subdict[1][node_id[-1]]
                else:
                    self.errors["exploration"][tuple([0] + node_id)] = f"Exploration is jumping steps. {node_id[-1]} does not exist in {subdict[1]}."
                    exploration_error_on_final_node = 1
            if exploration_error_on_final_node == 0:
                final_operation = operations[-1][0]
                predicted_result = int(final_operation.split("=")[1])
                matches = re.search(r"(\d+)([-+/*])(\d+)", final_operation.split("=")[0])
                if subdict[1]:
                    max_leaf_index = max(subdict[1].keys())
                    self.path = [0] + node_id + [max_leaf_index + 1]
                    if matches.group(2) == "/" and matches.group(3) == "0":
                        self.errors["arithmetic"][tuple(self.path)] = f"Invalid final operation: {final_operation}."
                        return
                    if predicted_result == eval(final_operation.split("=")[0]) and predicted_result == self.target:
                        self.final_op_correct = 1
                    if predicted_result != eval(final_operation.split("=")[0]) and predicted_result == self.target:
                        self.errors["arithmetic"][tuple(self.path)] = f"Invalid final operation: {final_operation}."
                        return
                    if self.final_op_correct == 1:
                        final_op_node = SearchTreeNode([target], final_operation, tuple(self.path))
                        subdict[1][max_leaf_index + 1] = [final_op_node, dict()]
                else:
                    self.path = [0] + node_id + [0]
                    if matches.group(2) == "/" and matches.group(3) == "0":
                        self.errors["arithmetic"][tuple(self.path)] = f"Invalid final operation: {final_operation}."
                        return
                    if predicted_result == eval(final_operation.split("=")[0]) and predicted_result == self.target:
                        self.final_op_correct = 1
                    if predicted_result != eval(final_operation.split("=")[0]) and predicted_result == self.target:
                        self.errors["arithmetic"][tuple(self.path)] = f"Invalid final operation: {final_operation}."
                        return
                    if self.final_op_correct == 1:
                        final_op_node = SearchTreeNode([target], final_operation, tuple(self.path))
                        subdict[1][0] = [final_op_node, dict()]

        if self.goal_reached == 1 and self.final_op_correct == 1:
            nodes_in_path = []
            for i in range(1, len(self.path)):
                nodes_in_path.append(self.path[: i + 1])
            for error_dict in self.errors.values():
                for node_id in nodes_in_path:
                    if tuple(node_id) in error_dict:
                        self.correctness = -1
            if self.correctness == -1:
                self.correctness = 0
            else:
                self.correctness = 1

        if self.correctness == 1:
            self.num_nodes_in_correct_path = len(self.path)
            self.rating = self.correctness - (self.num_total_nodes / self.max_nodes)

    def get_all_valid_nodes(self, data, set_valid_nodes):
        if data == self.data:
            set_valid_nodes.add(tuple(sorted(list(data[0][0].options))))
            self.get_all_valid_nodes(data[0][1], set_valid_nodes)
        else:
            for key, node_and_dict in data.items():
                if not any(
                    node_and_dict[0].id in error_type
                    for error_type in self.errors.values()
                    if isinstance(error_type, dict)
                ):
                    if isinstance(node_and_dict[0].options, list):
                        set_valid_nodes.add(tuple(sorted(node_and_dict[0].options)))
                    else:
                        set_valid_nodes.add(tuple(sorted([node_and_dict[0].options])))
                if bool(node_and_dict[1]):
                    self.get_all_valid_nodes(node_and_dict[1], set_valid_nodes)


def get_node_alignment(search_tree, symbolic_search_tree) -> float:
    valid_nodes_t1 = set()
    valid_nodes_t2 = set()
    search_tree.get_all_valid_nodes(search_tree.data, valid_nodes_t1)
    symbolic_search_tree.get_all_valid_nodes(symbolic_search_tree.data, valid_nodes_t2)
    intersection = valid_nodes_t1.intersection(valid_nodes_t2)
    max_nodes = max(len(valid_nodes_t1), len(valid_nodes_t2))
    if max_nodes == 0:
        return 0.0
    return len(intersection) / max_nodes


# ==============================================================================
# Search Algorithms (BFS & DFS)
# ==============================================================================

def bfs(target, nums, beam_size, heuristic=sum_heuristic):
    """
    Beam-search implementation producing a backtracking trace string.
    """
    search_trace = ""
    open_set = []
    
    # Push the initial node with its index, heuristic value, and parent index
    heapq.heappush(
        open_set,
        (
            heuristic(nums, target),
            CountdownNode(0, None, nums, [], heuristic(nums, target)),
        ),
    )
    node_index = 1

    while open_set:
        current_nodes = [
            heapq.heappop(open_set) for _ in range(min(beam_size, len(open_set)))
        ]
        if not current_nodes:
            break

        for idx, (_, current_node) in enumerate(current_nodes):
            search_trace += f"Current State: {target}:{current_node.nums}, Operations: {current_node.operations}\n"
            generated_nodes = []
            
            # Generate successors
            for i, j in itertools.combinations(range(len(current_node.nums)), 2):
                for result, operation in combine_nums(
                    current_node.nums[i], current_node.nums[j]
                ):
                    new_nums = [
                        current_node.nums[k]
                        for k in range(len(current_node.nums))
                        if k != i and k != j
                    ] + [result]
                    new_operations = current_node.operations + [operation]
                    new_heuristic = heuristic(new_nums, target)
                    new_node = CountdownNode(
                        node_index,
                        current_node,
                        new_nums,
                        new_operations,
                        new_heuristic,
                    )
                    generated_nodes.append((new_heuristic, new_node))
                    node_index += 1

            # Explicit Pruning: Keep only top 'beam_size' nodes
            generated_nodes.sort(key=lambda x: x[0])
            generated_nodes = generated_nodes[:beam_size]

            random.shuffle(generated_nodes)
            
            node_idx = 0
            for node_tuple in generated_nodes:
                node = node_tuple[-1]
                node.idx = f"{node.parent.idx},{node_idx}"
                operation = node.operations[-1]
                current_nums = node.nums
                search_trace += f"Exploring Operation: {operation}, Resulting Numbers: {current_nums}\n"
                
                if len(node.nums) == 1 and node.nums[0] == target:
                    search_trace += f"{node.nums[0]},{target} equal: Goal Reached\n"
                    return search_trace
                elif len(node.nums) == 1:
                    search_trace += f"{node.nums[0]},{target} unequal: No Solution\n"
                else:
                    search_trace += f"Generated Node #{node.idx}: {target}:{node.nums} Operation: {operation}\n"
                    node_idx += 1
            
            generated_nodes.sort(key=lambda x: x[0])
            for node_tuple in generated_nodes:
                heapq.heappush(open_set, node_tuple)

            if idx < len(current_nodes) - 1:
                _, next_node = current_nodes[idx + 1]
                next_index = next_node.idx
                search_trace += f"Moving to Node #{next_index}\n"

        if current_nodes:
            next_node_to_explore = open_set[0] if open_set else None
            if next_node_to_explore:
                _, next_node = next_node_to_explore
                next_index = next_node.idx
                search_trace += f"Moving to Node #{next_index}\n"

    search_trace += "No solution found."
    return search_trace


def dfs(target, nums, heuristic=sum_heuristic, threshold=None, search_trace="", open_set=None):
    """
    Depth-first search implementation producing a backtracking trace string.
    """
    if open_set is None:
        open_set = []
        open_set.append(
            (
                heuristic(nums, target),
                CountdownNode(0, None, nums, [], heuristic(nums, target)),
            )
        )

    while open_set:
        open_set.sort(key=lambda x: -x[0])
        _, current_node = open_set.pop()
        search_trace += f"Current State: {target}:{current_node.nums}, Operations: {current_node.operations}\n"

        generated_nodes = []
        for i, j in itertools.combinations(range(len(current_node.nums)), 2):
            node_index = 0
            for result, operation in combine_nums(
                current_node.nums[i], current_node.nums[j]
            ):
                new_nums = [
                    current_node.nums[k]
                    for k in range(len(current_node.nums))
                    if k != i and k != j
                ] + [result]
                new_operations = current_node.operations + [operation]
                new_heuristic = heuristic(new_nums, target)
                new_node = CountdownNode(
                    node_index, current_node, new_nums, new_operations, new_heuristic
                )
                generated_nodes.append((new_heuristic, new_node))

        kept_nodes = []
        for g in generated_nodes:
            if threshold is None or g[0] <= threshold:
                kept_nodes.append(g)
        generated_nodes = kept_nodes
        generated_nodes.sort(key=lambda x: x[0])

        node_index = 0
        for g, (_, new_node) in enumerate(generated_nodes):
            new_node.idx = f"{new_node.parent.idx},{node_index}"
            search_trace += f"Exploring Operation: {new_node.operations[-1]}, Resulting Numbers: {new_node.nums}\n"
            
            if len(new_node.nums) == 1 and new_node.nums[0] == target:
                search_trace += f"{new_node.nums[0]},{target} equal: Goal Reached\n"
                return search_trace
            elif len(new_node.nums) == 1:
                search_trace += f"{new_node.nums[0]},{target} unequal: No Solution\n"
            else:
                search_trace += f"Generated Node #{new_node.idx}: {target}:{new_node.nums} Operation: {new_node.operations[-1]}\n"
                new_set = [(new_heuristic, new_node)]
                search_trace += f"Moving to Node #{new_node.idx}\n"
                search_trace = dfs(
                    target,
                    nums,
                    heuristic=heuristic,
                    threshold=threshold,
                    search_trace=search_trace,
                    open_set=new_set,
                )
                if "Goal Reached" in search_trace:
                    return search_trace
            node_index += 1
            if g < len(generated_nodes) - 1:
                next_index = new_node.parent.idx
                search_trace += f"Moving to Node #{next_index}\n"
                search_trace += f"Current State: {target}:{new_node.parent.nums}, Operations: {new_node.parent.operations}\n"

        if open_set:
            next_node = open_set[-1][1]
            next_index = next_node.idx
            search_trace += f"Moving to Node #{next_index}\n"

    return search_trace


# ==============================================================================
# CountDown Solver & Problem Generator
# ==============================================================================

class CountDown(object):
    def __init__(
        self, max_target=100, start_size=4, min_target=10, max_intermediate=10000
    ):
        self.max_target = max_target
        self.min_target = min_target
        self.start_size = start_size
        self.max_intermediate = max_intermediate

    def generate(self, target):
        """
        Generates starting numbers and a valid equations solution to reach the target.
        """
        if target > self.max_target:
            raise ValueError("Target cannot be greater than max target")
        if target < self.min_target:
            raise ValueError("Target cannot be less than min target")

        found = False
        while not found:
            # Generate starting numbers up to target-1
            nums = [
                random.randint(1, target - 1) for _ in range(self.start_size)
            ]
            solution = self.search(target, nums)
            if solution is not None:
                found = True
        return nums, solution

    def search(self, target, nums, operations=None):
        """
        DFS search to find a sequence of valid equations to reach the target.
        """
        if operations is None:
            operations = []

        if len(nums) == 1:
            if nums[0] == target:
                return operations
            else:
                return None

        for i, j in itertools.combinations(range(len(nums)), 2):
            num1, num2 = nums[i], nums[j]
            remaining_nums = [nums[k] for k in range(len(nums)) if k != i and k != j]
            for result, operation in combine_nums(num1, num2, self.max_intermediate):
                new_nums = remaining_nums + [result]
                new_operations = operations + [operation]
                solution = self.search(target, new_nums, new_operations)
                if solution is not None:
                    return solution
        return None

    def convert_to_sequence(self, target, nums, solution, num_pause_tokens=8, pause_token="<pause>"):
        """
        Converts the numbers, target, and solution to the training data sequence format.
        Optionally inserts pause tokens after the target number.
        
        Example format: 90,8,20,50,24|<pause><pause><pause><pause><pause><pause><pause><pause>90*8=720,50-20=30,720/30=24
        """
        sequence = ",".join(str(num) for num in nums)
        sequence += f",{target}|"
        if num_pause_tokens > 0:
            sequence += pause_token * num_pause_tokens
        
        for i, operation in enumerate(solution):
            if i == 0:
                sequence += f"{operation}"
            else:
                sequence += f",{operation}"
        return sequence

    def convert_to_path(self, target, nums, operations):
        """
        Converts solution to readable backtracking path/search trace.
        """
        operations_explored = []
        search_trace = f"Current State: {target}:{nums}, Operations: {operations_explored}\n"
        node_index = 1
        for operation in operations:
            if "+" in operation:
                i, j = operation.split("=")[0].split("+")
                i, j = int(i), int(j)
                result = i + j
            elif "-" in operation:
                i, j = operation.split("=")[0].split("-")
                i, j = int(i), int(j)
                result = i - j
            elif "*" in operation:
                i, j = operation.split("=")[0].split("*")
                i, j = int(i), int(j)
                result = i * j
            elif "/" in operation:
                i, j = operation.split("=")[0].split("/")
                i, j = int(i), int(j)
                result = i // j  # Integer division

            new_nums = [
                int(nums[k]) for k in range(len(nums)) if nums[k] != i and nums[k] != j
            ] + [result]
            nums = new_nums
            search_trace += f"Exploring Operation: {operation}, Resulting Numbers: {nums}\n"
            if len(nums) == 1:
                search_trace += f"{nums[0]},{target} equal: Goal Reached\n"
            else:
                node_index += 1
                search_trace += f"Generated Node #{node_index}: {new_nums} from Operation: {operation}\n"
                operations_explored.append(operation)
                search_trace += f"Current State: {target}:{nums}, Operations: {operations_explored}\n"
        return search_trace


# ==============================================================================
# CLI Data Generation Runner
# ==============================================================================

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Countdown dataset generator with NextLat pause token integration.")

    # Data args
    parser.add_argument("--seed", type=int, default=0, help="Random seed")
    parser.add_argument("--data_dir", type=str, default="data/countdown", help="Directory to store data")

    # Countdown specific
    parser.add_argument("--start_range", type=int, default=4, help="Max Range of starting numbers")
    parser.add_argument("--min_range", type=int, default=4, help="Min Range of starting numbers")
    parser.add_argument("--min_target", type=int, default=10, help="Minimum target number")
    parser.add_argument("--max_target", type=int, default=100, help="Maximum target number")
    parser.add_argument("--num_samples", type=int, default=1000, help="Number of training data samples to generate")
    parser.add_argument("--max_intermediate", type=int, default=10000, help="Maximum value of intermediate numbers")

    # Pause tokens
    parser.add_argument("--num_pause_tokens", type=int, default=8, help="Number of pause tokens to insert")
    parser.add_argument("--pause_token", type=str, default="<pause>", help="String for pause token")

    # Search args (for exploration traces, if needed)
    parser.add_argument("--search", type=str, default="random", help="Search type for evaluation (astar, dfs, bfs, random)")
    parser.add_argument("--grow", action="store_true", help="Grow mode on or off, only new train set is created")
    parser.add_argument("--offset", type=int, default=1, help="Offset for random seed in grow mode")

    args = parser.parse_args()

    # Set random seed
    random.seed(args.seed)
    target_nums = [i for i in range(args.min_target, args.max_target + 1)]

    # Out-of-distribution target partition (reserve 10% for out-of-distribution evaluation)
    random.shuffle(target_nums)
    val_target_nums = target_nums[: len(target_nums) // 10]
    train_nums = target_nums[len(target_nums) // 10 :]
    
    print(f"Validation target numbers (OOD): {val_target_nums}")
    print(f"Train/In-distribution target numbers: {train_nums}")

    if args.grow:
        splits = ["grow"]
        target_list = [train_nums]
        # To avoid reusing the same samples from the train set, change the seed
        random.seed(args.seed + args.offset)
    else:
        splits = ["train", "val_target", "val"]
        target_list = [train_nums, val_target_nums, train_nums]

    total_samples = {3: 0, 4: 0, 5: 0}
    data_samples = {}

    for split, target_nums_split in zip(splits, target_list):
        data_samples[split] = []
        if split == "train" or split == "grow":
            num_samples = args.num_samples
        else:
            # 10% target is validation evaluation target
            num_samples = args.num_samples // 10 if args.num_samples >= 10 else 1

        print(f"Generating split '{split}' with {num_samples} samples...")
        
        zero_count = 0
        max_intermediate_observed = 0
        max_operations_observed = 0
        
        for _ in tqdm.tqdm(range(num_samples)):
            start_size = random.randint(args.min_range, args.start_range)
            cd = CountDown(
                max_target=args.max_target,
                start_size=start_size,
                min_target=args.min_target,
                max_intermediate=args.max_intermediate
            )
            
            target = random.choice(target_nums_split)
            nums, solution = cd.generate(target)
            
            max_intermediate_observed = max(
                max_intermediate_observed,
                max(int(operation.split("=")[1]) for operation in solution),
            )
            max_operations_observed = max(max_operations_observed, len(solution))
            
            sequence = cd.convert_to_sequence(
                target, nums, solution, 
                num_pause_tokens=args.num_pause_tokens, 
                pause_token=args.pause_token
            )
            
            # For validation split, avoid exact duplicates from train set
            if split == "val" and "train" in data_samples:
                while sequence in data_samples["train"]:
                    target = random.choice(target_nums_split)
                    nums, solution = cd.generate(target)
                    sequence = cd.convert_to_sequence(
                        target, nums, solution,
                        num_pause_tokens=args.num_pause_tokens,
                        pause_token=args.pause_token
                    )
            
            data_samples[split].append(sequence)
            if start_size in total_samples:
                total_samples[start_size] += 1

        print(f"Split {split} generated. Max intermediate observed: {max_intermediate_observed}, Max operations observed: {max_operations_observed}")

    # Write splits to output directory
    os.makedirs(args.data_dir, exist_ok=True)
    for split in splits:
        output_file = f"{args.data_dir}/{split}{args.offset if args.grow else ''}_b{args.start_range}_t{args.max_target}_n{args.num_samples}.txt"
        with open(output_file, "w") as f:
            for sample in data_samples[split]:
                f.write(f"{sample}\n")
        print(f"Saved {len(data_samples[split])} samples to {output_file}")